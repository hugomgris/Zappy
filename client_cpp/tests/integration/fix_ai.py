...
with open("/home/jareste/hugo/zappy/client_cpp/srcs/app/SimpleAI.cpp", "r") as f:
    text = f.read()
...
# 2. Fix requestVision
old_vision = r'void SimpleAI::requestVision\(\) \{\s*_lastVoirTime = std::chrono::duration_cast<std::chrono::milliseconds>\(\s*std::chrono::steady_clock::now\(\)\.time_since_epoch\(\)\s*\)\.count\(\);\s*_sender\.sendVoir\(\);\s*\}'
new_vision = '''void SimpleAI::requestVision() {
...
old_inv = r'void SimpleAI::requestInventory\(\) \{\s*_lastInventaireTime = std::chrono::duration_cast<std::chrono::milliseconds>\(\s*std::chrono::steady_clock::now\(\)\.time_since_epoch\(\)\s*\)\.count\(\);\s*_sender\.sendInventaire\(\);\s*\}'
new_inv = '''void SimpleAI::requestInventory() {
...
with open("/home/jareste/hugo/zappy/client_cpp/srcs/app/SimpleAI.cpp", "w") as f:
    f.write(text)
